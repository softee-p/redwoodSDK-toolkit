...
import { Aside, Badge } from "@/app/components/mdx";
...
// [!code word:initClientNavigation]
...
```tsx title="src/client.tsx"
history.scrollRestoration = "manual"; // [!code highlight]
```
...
```tsx title="src/client.tsx"
initClientNavigation({
  scrollToTop: false, // [!code highlight]
});
```
