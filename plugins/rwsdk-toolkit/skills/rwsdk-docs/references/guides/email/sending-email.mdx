...
import { Aside, Steps, FileTree } from "@/app/components/mdx";
...
2. Once you’ve created an account, you’ll be redirected to a page with instructions for sending your first email.
    ...
    <Aside type="note" title="Environmental Variables">
      You can find more information about [Environmental Variables here.](/core/env-vars)
    </Aside>
...
```tsx title="src/app/auth/actions.ts"
...
```tsx
// [!code word:react\:]
const Email = ({ name }: { name: string }) => { // [!code highlight:3]
...
const { data, error } = await resend.emails.send({
  ...
  to: email,
  subject: "👋 Hello World",
  react: <Email name="John" />, // [!code highlight]
});
```
...
const { data, error } = await resend.emails.send({
  ...
  to: email,
  subject: "👋 Hello World",
  html: "<h1>Hello World</h1>", // [!code highlight]
});
```
...
...
const { data, error } = await resend.emails.send({
  from: CONSTANTS.FROM_EMAIL, // [!code highlight]
  to: email,
  subject: "👋 Hello World",
  text: `Hello World`,
});
