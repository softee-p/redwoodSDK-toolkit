...
import { Aside, Tabs, TabItem, LinkCard, Badge } from "@/app/components/mdx";
...
```tsx title="src/app/pages/todos/TodoPage.tsx"
// [!code word:async]
...
```tsx title="@/pages/todos/functions.tsx"
...
```tsx title="@/pages/todos/AddTodo.tsx"
"use client"; // [!code highlight]
...
```tsx title="src/pages/todos/functions.tsx"
"use server";
...
export async function addTodo(formData: FormData) {
  ...
  return Response.redirect("/todos", 303); // [!code highlight]
}
```
