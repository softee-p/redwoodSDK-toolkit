...
| [references/migrating.mdx](${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/migrating.mdx) | Upgrading 0.x to 1.x, breaking changes, peerDependencies migration (`react-server-dom-webpack`), `compatibility_date` `2025-08-21`, `requestInfo.response.headers` change, RSC actions through middleware pipeline, `isAction` flag, `resolveSSRValue` removal, D1/Prisma to SQLite DO passkey migration, `standard` starter removal |
...
| [references/core/routing.mdx](${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/core/routing.mdx) | `defineApp`, `route`, middleware, `ctx`, interrupters, static/parameter/wildcard routes, query parameters, `searchParams`, HTTP methods, `custom` methods, `config.disableOptions`/`config.disable405`, explicit HEAD handling, `render()`, `requestInfo`, `getRequestInfo()`, `DefaultAppContext` type extension, `linkFor`, prefetch, generation-based cache eviction, named `app` export pattern for Cron/Queues with `linkFor`, migration from route constants to `linkFor` |
| [references/core/react-server-components.mdx](${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/core/react-server-components.mdx) | RSC, `"use client"`, `"use server"`, `serverQuery`, `serverAction` (with `{ method: "GET" }` option), middleware arrays, `x-rsc-data-only` header, Response returns (redirects), Suspense, streaming, `renderToStream`/`renderToString` options (`Document`, `injectRSCPayload`, `onError`), `onActionResponse` |
| [references/core/authentication.mdx](${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/core/authentication.mdx) | Session management, `defineDurableSession`, Durable Objects, passkey addon, cookies, `sessionStore` (`load`/`save`/`remove` methods), `UserSession` DO class pattern (`getSession`/`saveSession`/`revokeSession`), middleware auth, `ErrorResponse`, server action auth flow, per-route interruptors |
| [references/core/security.mdx](${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/core/security.mdx) | CSP, nonce (`rw.nonce`), `RouteMiddleware` type, `img-src` directive, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, `setCommonHeaders`, `response.headers`, device permission lifting (`geolocation`/`microphone`/`camera` with `self`) |
...
- **Device permissions, camera, microphone** -> `${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/core/security.mdx`
- **Sentry, error monitoring** -> `${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/guides/frontend/error-handling.mdx` + `${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/reference/sdk-router.mdx`
- **Prefetching, cache management** -> `${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/guides/frontend/client-side-nav.mdx`
- **Queue/cron request context errors** -> `${CLAUDE_PLUGIN_ROOT}/skills/rwsdk-docs/references/guides/troubleshooting.mdx`
